...
`setTimeout_ms(timeout_ms)` samt `hasTimedOut()`. Den riktiga implementationen i det delade
[ATmega328p-övningsbiblioteket](../../../libs/atmega/README.md), `driver::timer::Atmega328p`,
tar dessutom emot en valfri callback (`void (*)()`) som konstruktorargument, för att köras vid
timeout, den är inte en del av `Interface` utan specifik för den konkreta implementationen.
...
garanterar exakt när er kod får köra igen.
...
`driver::timer::Atmega328p` räknar avbrott i mjukvara: varje avbrott motsvarar ett fast
intervall (0,128 ms), och timeout inträffar när tillräckligt många avbrott räknats. Det är den
omräkningen mellan millisekunder och antal avbrott som testfilens `getMaxCount()` gör, och det
är där ni ska leta om ett timeout-test blir rött.
