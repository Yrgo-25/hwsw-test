...
select_files() {
    local -n out=$1
    mapfile -t out < <(find * -path "libs/test" -prune -o \
...
select_python_files() {
    local -n out=$1
    mapfile -t out < <(find * -path "libs/test" -prune -o -name "*.py" -print)
}
...
format_files() {
    ...
    # Fail loudly if nothing was selected: clang-format with no file arguments reads stdin,
    # which is /dev/null in CI, so it would exit 0 having checked nothing at all.
    if [[ ${#files[@]} -eq 0 ]]
    then
        echo "error: no C/C++ files found. Check the file patterns in select_files()." >&2
        exit 1
    fi
